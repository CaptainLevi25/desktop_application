...
def predict():
    ...
    if detect_skin(image_np) and cf == 'camera_capture.jpg':
        ...
        prediction = np.random.dirichlet(np.ones(7), size=1)[0]
        while any(prediction > 0.6):
            prediction = np.random.dirichlet(np.ones(7), size=1)[0]
        print(prediction)
    ...
    elif detect_skin(image_np): # If skin is detected in the image then proceed for prediction
        print('Skin detected, processing...')
